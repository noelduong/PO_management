...
let code = fs.readFileSync('d:/ORDER_APP/ORDER_APP/Code.gs', 'utf-8');

const old_row_assign = `      // Khởi tạo dòng dữ liệu tương ứng với số lượng cột hiện tại
...
const new_row_assign = `      // Khởi tạo dòng dữ liệu tương ứng với số lượng cột hiện tại
      const rowData = new Array(currentHeaders.length).fill("");
...
const old_header_logic = `    // Quét qua tất cả các size để thêm cột mới vào header nếu cần
    const neededSizes = new Set();`;

const new_header_logic = `    // Thêm Link Ảnh nếu chưa có
...
const old_dopost = `    if (payload.action === 'saveOrder' || payload.action === 'updateOrder') {`;

const new_dopost = `    if (payload.action === 'deleteOrder') {
...
if (!code.includes("function deleteOrderData")) {
    code += delete_func;
}

fs.writeFileSync('d:/ORDER_APP/ORDER_APP/Code.gs', code, 'utf-8');
console.log('Done!');
